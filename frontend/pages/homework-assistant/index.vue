  ...
  <PageWrapper title="Homework Assistant">
    ...
    >
      <Card v-if="!fileUploaded" class="max-w-1/2 overflow-hidden">
        ...
        <CardFooter>
          <Button @click="fileUploaded = true">
            <LucideUpload /> Upload file
          </Button>
        </CardFooter>
      </Card>
    </transition>
    <div class="flex flex-col h-[calc(100vh-4rem)] overflow-hidden space-y-4">
      <!-- Explanation Section -->
      <Card class="overflow-y-auto max-h-64">
        <CardHeader>
      ...
      <!-- Chat Section -->
      <div class="flex flex-col flex-1 bg-white dark:bg-gray-900 rounded-lg shadow-md overflow-hidden">
        <!-- Chat Messages -->
        ...
        <div class="px-4 py-3 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 flex items-center space-x-2">
          <Textarea
            ...
            @input="autoResize"
          />
          <Button variant="primary" :disabled="!draft || loading" @click="sendMessage">
            Send
          </Button>
